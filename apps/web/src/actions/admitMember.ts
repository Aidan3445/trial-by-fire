...
/**
  * Admit a member to the league
  * @param hash Hash of the league to delete the member from
  * @param memberId The ID of the member to admit
  * @throws an error if the member cannot be admitted
  * @returns Success status of the update
  * @returnObj `{ success }`
  */
export default async function admitMember(hash: string, memberId: number) {
  try {
    return await requireLeagueAdminAuth(admitMemberLogic)(hash, memberId);
  } catch (e) {
    let message: string;
